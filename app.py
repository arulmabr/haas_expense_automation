...
ADDRESS_LABELS = re.compile(
    ...
    r'|^\s*(?:Check.?in|Check.?out)\s+(?:at|location|address)'  # Line-start only
    r'|^\s*(?:Location|Destination|Address|Venue)\s*[:\-]'  # Line-start labels with delimiter
    r')',
...
Return ONLY the business purpose statement, nothing else."""
    ...
    def _normalize_page_sizes(self, pdf_bytes: bytes) -> bytes:
        """Scale non-Letter pages to Letter so PDF viewers don't clip mixed sizes."""
        ...
        try:
            ...
            for i in range(len(src)):
                ...
                is_letter = (abs(w - LETTER_W) < TOLERANCE and
                             abs(h - LETTER_H) < TOLERANCE)
                if is_letter:
                    dst.insert_pdf(src, from_page=i, to_page=i)
                else:
                    # Scale entire page to fit Letter, preserving aspect ratio
                    new_page = dst.new_page(width=LETTER_W, height=LETTER_H)
                    scale = min(LETTER_W / w, LETTER_H / h)
                    sw, sh = w * scale, h * scale
                    x = (LETTER_W - sw) / 2
                    y = (LETTER_H - sh) / 2
                    new_page.show_pdf_page(
                        fitz.Rect(x, y, x + sw, y + sh), src, i
                    )
            result = dst.tobytes()
            src.close()
            dst.close()
        ...
        for i, line_info in enumerate(all_lines):
            if ADDRESS_LABELS.search(line_info["text"]):
                ...
                while addr_lines_found < ADDRESS_LABEL_MAX_LINES and i + j < len(all_lines):
                    ...
                    # Stop at another label, an amount, empty text, or prose
                    if (not next_text
                            or ADDRESS_LABELS.search(next_text)
                            or AMOUNT_LINE_PATTERN.match(next_text)
                            or len(next_text) > 80):  # prose lines, not addresses
                        break
    ...
    def generate_xlsx_summary(self, expenses: list, metadata: dict) -> bytes:
        ...
        def _safe(val):
            """Strip characters that openpyxl considers illegal for Excel."""
            return ILLEGAL_CHARACTERS_RE.sub("", val) if isinstance(val, str) else val

        # ── Fill header fields ──
        name = f"{metadata.get('first_name', '')} {metadata.get('last_name', '')}".strip()
        ws["B1"] = _safe(name)
        ws["B2"] = _safe(metadata.get("description", ""))
        ...
        for section_key, section_info in self.XLSX_SECTIONS.items():
            ...
            for i, (expense, amount_usd) in enumerate(items):
                ...
                ws.cell(row=row, column=4, value=_safe(expense.description))  # D: Notes
    ...
    def upload_files_to_s3(self, files_data: Dict[str, bytes], merged_pdf: bytes = None) -> tuple:
        ...
        if not merged_pdf:
            ...
            if merged_pdf:
                merged_pdf = self._normalize_page_sizes(merged_pdf)
